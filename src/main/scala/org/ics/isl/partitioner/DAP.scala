...
object DAP {
    ...
    def main(args: Array[String]): Unit = {
        ...
        if(args.size != 5){
            ...
            println("-->Arguments: dataset name, number of partitions, hdfs base path, insances path, schema path")
            System.exit(-1)
        }
      ...
      var timeDAP = System.currentTimeMillis - timeStart;
  ...
  //(node =>(node._1, dependence(importanceMap, node._2, maxDiff, minDiff)))
    def createClusters(dependences: Seq[(Long, Map[Long, (Double, Seq[(Long, Long, String)])])],
     ...
     // val test = centroidMap.map{case(cID, dependences) => (cID, dependences.map(d => d._2) )}
     // test.foreach{case(cID, nodes) => println("-- centroid- "+cID +": "+ nodes.mkString(","))}
      ...
      var leftoutSize = leftoutNodes.map(schemaVertices(_)).map(schemaNodeCount.getOrElse(_, 0.toLong)).reduce(_+_)
    ...
    */
    def cardinalityCloseness(triplet: EdgeTriplet[String, (String, Double)], c: Long, cc: Map[(String, String, String), (Long, Long)]): Double = {
    ...
    * weight: (destBC/CC)
    */
    def addWeight(c: Long ,triplet: EdgeTriplet[String, (String, Double)], importance: Map[Long, Double], cc: Map[(String, String, String), (Long, Long)]): Edge[(String, Double)] = {
